...
def registrar_consulta_juridica(
    ...
    """
    Grava uma consulta do Assistente Jurídico no Supabase.

    A tabela historico_consultas_juridicas deve existir no Supabase.
    """
...
def obter_historico_consulta_juridica(
    consulta_id: Any
) -> Optional[Dict[str, Any]]:
    """
    Recupera uma consulta específica do histórico.
    """
    consulta_id_q = quote(str(consulta_id), safe="")

    registros = _request(
        "GET",
        f"{_endpoint(SUPABASE_HISTORICO_JURIDICO_TABLE)}"
        f"?id=eq.{consulta_id_q}&select=*&limit=1",
        headers=_headers(),
    ) or []
...
def buscar_contexto_ativo_pi(
    ...
    """
    Busca dados da base de ativos_pi para que o Assistente Jurídico
    possa receber contexto institucional do cadastro do IFSC.
    """
    try:
    ...
    if ativo_pi_id is not None and str(ativo_pi_id).strip():
        filtro = f"id=eq.{quote(str(ativo_pi_id), safe='')}"
    else:
        ...
        filtro = (
            f"or=("
            f"numero_processo.ilike.*{termo_q}*,"
...
def obter_obrigacoes_ativo(
    ativo_pi_id: Any,
    limite: int = 100,
) -> pd.DataFrame:
    """
    Recupera as obrigações financeiras vinculadas a uma PI.
    """
    try:
        limite = max(1, min(int(limite), 200))
    except Exception:
    ...
    ativo_q = quote(str(ativo_pi_id), safe="")

    url = (
        f"{_endpoint('obrigacoes_financeiras')}"
        f"?select=*"
        f"&ativo_pi_id=eq.{ativo_q}"
        f"&order=data_vencimento.asc"
        f"&limit={limite}"
    )
