...
def data_para_input(valor):
    """Converte um valor para date, tratando valores vazios."""
    if valor is None:
        return None
...
def modalidade_pi(valor):
    return db.normalizar_modalidade(valor)


def status_pagamento(row):
    """Calcula o status atual de um pagamento."""
...
def montar_linhas_dashboard(df_pis, modalidade):
    ...
    for _, pi in df_pis.iterrows():
        ...
        for _, pgto in pagamentos.iterrows():
            ...
            inicio_ord = data_para_input(
                pgto.get("data_inicio_ordinario")
            )

            fim_ord = data_para_input(
...
def dashboard_modalidade(titulo, modalidade):
    ...
    if "modalidade_pi" not in df.columns:
        st.error(
            "A coluna 'modalidade_pi' não foi encontrada no banco de dados."
        )
        return

    df_tipo = df[
        df["modalidade_pi"].apply(modalidade_pi) == modalidade
    ].copy()
    ...
    if df_dash.empty:
        st.info(
            "Nenhum pagamento desta modalidade está "
            "em prazo ordinário neste momento."
        )
        return
...
# ============================================================
    ...
    if df is None or df.empty:
        ...
        busca = st.text_input(
            "🔍 Filtrar por processo, título, inventor, gestor, campus ou classificação:"
        )
        ...
        if df_filtrado.empty:
            ...
            st.warning(
                "Nenhuma PI correspondente aos filtros aplicados."
            )
            ...
            with st.form(
                ...
                with col_a:
                    ...
                    edit_modalidade = st.selectbox(
                        "Modalidade de PI",
                        modalidades,
                        index=indice_modalidade,
                    )

                    edit_data_dep = st.date_input(
                        "Data do Depósito",
                        value=data_para_input(
                            pi.get("data_deposito")
                        ),
                    )
                ...
                if salvar:
                    ...
                    ok, msg = db.atualizar_patente(
                        ...
                        data_dep=
                            edit_data_dep.strftime("%Y-%m-%d")
                            if edit_data_dep
                            else None,

                        data_conc=
                            edit_data_conc.strftime("%Y-%m-%d")
                            if edit_data_conc
                            else None,
                        ...
                        data_publicacao=
                            pi.get("data_publicacao"),

                        data_exame=
                            pi.get("data_exame"),

                        acordo_titularidade=
                            pi.get("acordo_titularidade"),
        ...
        if pagamentos is None or pagamentos.empty:
            ...
            st.warning(
                "Nenhum pagamento encontrado para esta PI."
            )
            ...
            with col4:
                ...
                if st.button(
                    ...
                    try:
                        ...
                        st.success(
                            "Pagamento marcado como não pagar."
                        )
...
# ============================================================
    ...
    st.title(
        "📤 Importar Propriedades Intelectuais do Excel"
    )
    ...
    if arquivo_excel:
        ...
        try:
            ...
            problemas = db.analisar_inconsistencias_excel(
                arquivo_excel
            )
            ...
            if problemas:
                ...
                st.success(
                    "Estrutura da planilha verificada com sucesso."
                )

        except Exception as exc:

            st.error(
                f"Erro ao verificar a planilha: {exc}"
            )
        ...
        if st.button(
            ...
            try:
                ...
                st.success(
                    ...
                    f"{sucessos} de {total} registros processados com sucesso."
                )

            except Exception as exc:

                st.error(
                    f"Erro durante a importação: {exc}"
                )
...
# ============================================================
    ...
    if df is None or df.empty:
        ...
        st.info(
            "Não há PIs cadastradas para gerar relatórios."
        )
        ...
        with col1:
            ...
            try:
                ...
                st.error(
                    f"Erro ao gerar relatório completo: {exc}"
                )
        ...
        with col2:
            ...
            try:
                ...
                st.error(
                    f"Erro ao gerar relatório de pagamentos: {exc}"
                )
        ...
        with col3:
            ...
            try:
                ...
                st.error(
                    f"Erro ao gerar relatório de alertas: {exc}"
                )
