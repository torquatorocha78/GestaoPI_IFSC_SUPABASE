    ...
    for _, pi in df_pis.iterrows():
        ...
        for _, pgto in pagamentos.iterrows():
            if pgto.get("status") == "nao_pagar" or pgto.get("data_pagamento"):
                continue
            inicio_ord = data_para_input(pgto.get("data_inicio_ordinario"))
            fim_ord = data_para_input(pgto.get("data_fim_ordinario"))
...
def dashboard_modalidade(titulo, modalidade):
    ...
    df_tipo = df[df["modalidade_pi"].apply(modalidade_pi) == modalidade].copy()
...
if pagina == "📊 Dashboard Patentes":
    ...
    if df.empty:
        ...
        if filtro_tipo != "Todas":
            df_filtrado = df_filtrado[df_filtrado["modalidade_pi"].apply(modalidade_pi) == filtro_tipo]
        if busca:
            termo = busca.lower()
        ...
        modalidade = modalidade_pi(pi.get("modalidade_pi"))
        ...
        with st.expander("✏️ Editar dados desta PI"):
            with st.form(f"form_editar_{pi_id}"):
                ...
                with col_a:
                    edit_id_externo = st.text_input("ID Externo", value=texto(pi.get("id_externo")))
                    edit_numero = st.text_input("Número do Processo", value=texto(pi.get("numero_patente")))
                    edit_titulo = st.text_input("Título", value=texto(pi.get("titulo")))
                    ...
                    edit_data_dep = st.date_input("Data do Depósito", value=data_para_input(pi.get("data_deposito")))
                    edit_data_conc = st.date_input("Data de Concessão", value=data_para_input(pi.get("data_concessao")))
                    edit_ano = st.number_input("Ano", value=int(pi.get("ano")) if pi.get("ano") else datetime.now().year)
                with col_b:
                    edit_titular = st.text_area("Depositante / Titular", value=texto(pi.get("titular")), height=80)
                    edit_inventores = st.text_area("Inventores", value=texto(pi.get("inventores")), height=80)
                    edit_gestor = st.text_input("Gestor", value=texto(pi.get("gestor")))
                    edit_status = st.text_input("Status", value=texto(pi.get("status")))
                    edit_campus = st.text_input("Campus", value=texto(pi.get("campus")))
                    edit_ipc = st.text_input("IPC / Classificação", value=texto(pi.get("ipc_classificacao")))

                edit_descricao = st.text_area("Resumo / Descrição", value=texto(pi.get("descricao")), height=120)
